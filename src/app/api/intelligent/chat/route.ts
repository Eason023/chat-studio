...
function createGlobalMemorySystemPrompt(
  ...
  return [
    ...
    "The three memory tiers must stay clearly separated. Do not store the same idea in multiple tiers.",
    "Before adding any new entry, first check whether the same idea is already captured anywhere in the current memory bank, even under a different tier. If it already exists in substance, do not add it again.",
    "Use userFeatures only for durable facts about the user: unusual long-term interests, stable personal preferences, recurring habits, background traits, or standing language/style preferences that describe the user rather than a single task.",
    "Never use userFeatures for one-off tasks, recent requests, current projects, temporary goals, or instructions about how to answer a particular class of question.",
    "Use instructionMemory only for explicit durable user instructions about how the assistant should behave in future turns, especially when the user clearly says what they want the assistant to do or avoid doing for a type of request.",
    "Instruction memory should contain rules like how to respond, what style to use, what level of detail to prefer, or what workflow the user expects for future answers.",
    "Never use instructionMemory for facts about what happened, what project was discussed, what website was analyzed, or what topic the user recently asked about. Those belong to recentEvents or nowhere.",
    "Use recentEvents only for notable current or near-term session-related facts: ongoing work, active projects, unresolved blockers, recently investigated items, or special events likely to matter again soon.",
    "Never use recentEvents for durable user traits or standing response instructions.",
